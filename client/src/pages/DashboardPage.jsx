  ...
  return (
    <DashboardLayout title="Dashboard" subtitle="Monitor marketplace activity, proposals, and active AI work.">
        ...
        {stats.map((stat) => (
          <Card key={stat.label} className="hover:shadow-soft">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className={`rounded-2xl bg-gradient-to-br ${stat.tone} px-3 py-2 text-sm font-bold text-white`}>{stat.trend}</div>
            </div>
          </Card>
        ))}
      </div>
        ...
        <Card className="hover:shadow-soft">
          ...
          <div className="space-y-4">
            {activities.map((activity) => (
              <div key={activity.title} className="flex gap-4 rounded-2xl bg-white/80 p-4">
                <div className="mt-1 grid h-10 w-10 flex-none place-items-center rounded-xl bg-indigo-50 text-indigo-600">
        ...
        <Card className="hover:shadow-soft">
          ...
          <div className="space-y-4">
            {requests.slice(0, 3).map((request) => (
              <div key={request.id} className="rounded-2xl border border-slate-100 bg-white/85 p-4">
                ...
                <div className="mt-4 flex items-center justify-between text-sm">
                  <span className="font-bold text-indigo-600">{request.budget}</span>
      ...
      <Card className="mt-6 hover:shadow-soft">
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-slate-100 bg-white/80">
