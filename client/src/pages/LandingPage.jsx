  ...
  return (
    <MarketingLayout>
      <section className="page-shell grid min-h-[calc(100vh-5rem)] items-center gap-12 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:py-20">
        <div>
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-indigo-100 bg-white/80 px-4 py-2 text-sm font-semibold text-indigo-700 shadow-soft">
          ...
          <div className="mt-10 grid grid-cols-3 gap-4 max-w-xl">
            {['500+ providers', '92% match rate', '24h avg reply'].map((item) => (
              <div key={item} className="rounded-2xl border border-white/80 bg-white/70 p-4 text-center shadow-soft backdrop-blur-xl">
        ...
        <div className="relative">
          <div className="glass rounded-2xl p-5">
            <div className="rounded-2xl bg-gradient-to-br from-indigo-600 via-blue-600 to-violet-600 p-6 text-white shadow-glow">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-indigo-100">Featured request</p>
                  <h2 className="mt-2 font-display text-2xl font-bold">AI revenue intelligence copilot</h2>
                </div>
                <WandSparkles className="h-9 w-9 text-indigo-100" />
              </div>
              <p className="mt-5 text-sm leading-6 text-indigo-50">Connect CRM, billing, and product data so operators can ask complex growth questions without waiting on analysts.</p>
              ...
              {['Proposal quality', 'Provider fit'].map((title, index) => (
                <div key={title} className="rounded-2xl bg-white p-5 shadow-soft">
                  ...
                  <div className="mt-4 h-2 rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-blue-500" style={{ width: index === 0 ? '92%' : '86%' }} />
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="page-shell py-12">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="font-semibold text-indigo-600">Why teams choose it</p>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {steps.map((step, index) => (
                <div key={step} className="rounded-2xl bg-white p-5 shadow-soft">
                  <div className="mb-5 grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-indigo-600 to-blue-600 font-bold text-white">{index + 1}</div>
          ...
          {testimonials.map((item) => (
            <Card key={item.name}>
              ...
              <div className="mt-6 flex items-center gap-3">
                <div className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 font-bold text-white">{item.name.slice(0, 2)}</div>
                <div>
      ...
      <section className="page-shell py-12">
        <div className="rounded-2xl bg-gradient-to-r from-slate-950 via-indigo-950 to-blue-900 p-8 text-white shadow-glow md:p-10">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="font-display text-3xl font-bold">Ready to make AI work feel organized?</h2>
              <p className="mt-3 max-w-2xl text-indigo-100">Start with a request, browse the marketplace, or jump into the provider dashboard.</p>
            </div>
            <Button to="/dashboard" variant="secondary" className="bg-white text-indigo-700">Open Dashboard <Zap className="h-4 w-4" /></Button>
          </div>
        </div>
      </section>
