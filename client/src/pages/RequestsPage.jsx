...
export default function RequestsPage() {
  ...
  return (
    <DashboardLayout title="Requests" subtitle="Browse AI service opportunities from teams ready to build.">
      <div className="mb-6 flex flex-col gap-3 rounded-lg border border-white/70 bg-white/75 p-4 shadow-soft backdrop-blur-xl md:flex-row md:items-center">
        <label className="relative flex-1">
          <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 pl-11 text-sm outline-none transition focus:border-blue-300 focus:ring-4 focus:ring-blue-100" placeholder="Search requests, skills, or clients" />
        </label>
